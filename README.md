...
同一低推理模型面对长篇、编号密集的技术材料时，单次回答通常会先做摘要，再组织结论；这能保留主旨，却容易压缩掉子项、关键标识和评估方法。SolutionScope 将任务拆成“**证据规划 → 覆盖清单 → 受约束回答 → 证据回绑**”，让模型必须逐项处置，而不是只写一段看起来完整的总结。

![SolutionScope 提升机制与开发集结果](docs/assets/04-why-it-improves.svg)

在 6 个冻结开发案例上，关键标识召回由 **74/111 提升至 111/111**，评估目标召回由 **37/70 提升至 70/70**；代价是 token 使用约为直接回答的 **1.95×**。证据绑定和验收边界两项在直接回答中已经满分，因此不宣称额外提升。

[查看完整对照设计、原因分析与口径边界 →](docs/evaluation-dev-pilot.md)
...
当前可公开复核的结果来自一次规则对齐的开发集对照：相同 `gpt-5.4-mini / low`、相同冻结证据池和 6 个案例。提升集中在清单完整性：关键标识 **+33.3 pp**、评估目标 **+47.1 pp**、评估方法 **+11.8 pp**；工作流使用两次模型调用，token 约为直接回答的 **1.95×**。

这只是 **DEV / n=6** 的机制验证，不能解释为最终准确率、专家一致性、泛化能力或业务 ROI。更严谨的结论是：证据规划与覆盖约束在这组案例中减少了低推理模型对长清单的压缩遗漏，但增加了推理成本。
